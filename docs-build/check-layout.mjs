...
for (const file of files) {
  ...
  const problems = await page.evaluate((contentH) => {
    ...
    for (const el of document.querySelectorAll(
      "td, th, p, li, div, figcaption, caption, dd, dt",
    )) {
    ...
    const blocks = [
      ...document.querySelectorAll(".page > section > *, .page > div > *"),
    ].filter((el) => {
      const cs = getComputedStyle(el);
      return (
        cs.position === "static" &&
    ...
    for (let i = 0; i < blocks.length - 1; i++) {
      ...
      if (b.top < a.bottom - 2 && b.height > 0 && a.height > 0) {
        out.push({
          kind: "BLOCKS OVERLAP",
          tag:
            blocks[i].tagName.toLowerCase() +
            " / " +
            blocks[i + 1].tagName.toLowerCase(),
  ...
  if (problems.length === 0) {
    ...
    for (const p of problems) {
      console.log(
        `  [${p.kind}] ${p.tag} — ${p.height}px vs limit ${p.limit}px`,
      );
      console.log(`      ${p.what}`);
    }
  }
