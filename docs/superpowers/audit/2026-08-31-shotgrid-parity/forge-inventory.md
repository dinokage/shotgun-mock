...
- **Data source:** Frontend-mock — standup posts (and the dailies playlist, feed approvals) are read/written via `useStandupsStore` (`@/store/standups`), a client-only, `localStorage`-persisted zustand store with no backend; the page's `apiClient.get("/standups")` poll is a decorative connectivity heartbeat only (no such route is mounted server-side) and doesn't feed any displayed data. Broadcasts are also frontend-mock (`useBroadcastsStore`); playlist/task data via `useTasksStore` (mock) and static `USERS`/`DEPARTMENTS`/`PROJECTS`.
...
**Correcting `forge-final.md` §6:** that document states only `/auth`, `/projects`, `/tasks`, `/users` are real, tenant-scoped, DB-backed API endpoints. As of the current `main` snapshot in this worktree, that list is **stale/out of date** — `artifacts/api-server/src/routes/index.ts` now also mounts `/departments` and `/roles`, each backed by a real Drizzle table (`departmentsTable`, `tenantRolesTable`) and real Express handlers that query Postgres (`routes/departments.ts`, `routes/roles.ts` — all read/write via `db.select()`/`db.insert()`, none are stubs). **Current real surface: 6 resource groups (`/auth/*`, `/projects`, `/tasks`, `/users`, `/departments`, `/roles`), not 4.** This reflects the RBAC/Admin-Panel hardening work referenced in the spec's inputs section.
...
|---|---|
| Projects, ProjectDetail (project record only), Tasks (top-level list only), Departments, AdminPanel | Login (auth is real; the portal UI itself has no data), ClientReview, DeliveryDetail, Home, ProjectDetail (all 5 tab bodies), Assets, AssetDetail, ShotDetail, DepartmentDetail, People, Profile, Review, Scheduling, Marketplace, PluginDetail, IntegrationsHub, Workflows, WorkflowEditor, WorkflowRun, SchemaBuilder, Publishing, Analytics, ProductionDashboard, FinancialDashboard, Audit, Settings, Deliveries, Chat, TrackingGrid, Timesheets, Notifications, DailyStandup |
