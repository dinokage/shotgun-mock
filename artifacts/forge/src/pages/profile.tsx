...
export default function Profile() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {id && (
        <Link href="/people" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors w-fit">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-6">
        <Avatar className="w-24 h-24 border-4 shadow-sm" style={{ borderColor: dept?.color || 'var(--border)' }}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <Badge variant={user.status === 'active' ? 'default' : 'secondary'} className={user.status === 'active' ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20 shadow-none' : ''}>
              {user.status}
            </Badge>
          </div>
          <p className="text-muted-foreground text-lg">{ROLE_LABELS[user.role] || user.title}</p>
          
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-3 text-sm text-muted-foreground">
            <span className="flex items-center gap-1.5"><Mail className="w-4 h-4" /> {user.email}</span>
            {dept && (
              <Link href={`/departments/${dept.id}`} className="flex items-center gap-1.5 hover:text-primary transition-colors">
                <Building2 className="w-4 h-4" /> {dept.name}
              </Link>
            )}
            <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4" /> {user.studioId === 'studio1' ? 'Portland' : user.studioId === 'studio2' ? 'London' : 'Tokyo'}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-6">
          ...
          {supervisor && (
            <Card className="border-border/50">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm text-muted-foreground uppercase tracking-wider">Reports To</CardTitle>
              </CardHeader>
              <CardContent>
                <Link href={`/people/${supervisor.id}`}>
                  <div className="flex items-center gap-3 p-2 -mx-2 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate">{ROLE_LABELS[supervisor.role] || supervisor.title}</div>
                    </div>
                  </div>
                </Link>
              </CardContent>
          ...
          <Card className="border-border/50">
             <CardHeader className="pb-3">
                <CardTitle className="text-sm text-muted-foreground uppercase tracking-wider">Stats</CardTitle>
              </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2 text-sm"><Clock className="w-4 h-4 text-yellow-500" /> Capacity</div>
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm text-muted-foreground uppercase tracking-wider">Skills & Software</CardTitle>
            </CardHeader>
            <CardContent>
                      ...
                      <div key={task.id} className="p-4 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => setActiveTaskDrawer(task.id)}>
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-semibold text-sm hover:text-primary transition-colors">{task.title}</span>
                          </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground ml-[92px]">
                          ...
                          <span>•</span>
                          <span style={{ color: dept?.color }}>{dept?.abbreviation} Phase</span>
                        </div>
                      </div>
                    ))}
              ...
              <Card className="border-border/50">
                ...
                <CardContent>
                  ...
                  <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground justify-center">
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-green-500 opacity-80" /> Available</div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-yellow-500 opacity-80" /> Near Capacity</div>
                    <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-red-500 opacity-80" /> Overloaded</div>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
