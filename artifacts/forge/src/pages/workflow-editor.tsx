...
const PALETTE_ITEMS: PaletteItem[] = [
  { id: 'trigger', label: 'Start / Trigger', description: 'Starting point for this workflow', icon: 'Play', color: 'green', kind: 'trigger' },
  { id: 'dept-stage', label: 'Dept Stage (2D/3D/VFX)', description: 'Create a department task stage', icon: 'Box', color: 'blue', kind: 'action' },
  { id: 'internal-review', label: 'Internal Review', description: 'Auto-schedule an internal review', icon: 'CheckSquare', color: 'purple', kind: 'action' },
  { id: 'client-feedback', label: 'Client Feedback', description: 'Request client feedback / approval', icon: 'UserPlus', color: 'orange', kind: 'action' },
  { id: 'branch', label: 'Branch / Condition', description: 'Branch based on a condition', icon: 'Workflow', color: 'cyan', kind: 'condition' },
];
...
function createNodeFromPaletteItem(item: PaletteItem, position: { x: number; y: number }): StoredWorkflowNode {
  return {
...
const WorkflowNode = ({ data, selected }: NodeProps<StoredWorkflowNode>) => {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} className="w-16 !bg-primary/50 transition-transform hover:scale-125" />
      ...
      <Handle type="source" position={Position.Bottom} className="w-16 !bg-primary transition-transform hover:scale-125" />
    </motion.div>
  );
};
 ...
 */
function simulateTestRun(nodes: StoredWorkflowNode[], edges: Edge[]): TestRunResult {
  ...
  return {
    ...
    steps: order.map((n) => ({ id: n.id, label: n.data.label, icon: n.data.icon, color: n.data.color, kind: n.data.kind })),
    unreachable,
  };
}
...
function WorkflowEditorInner() {
  ...
  const [nodes, setNodes, onNodesChangeBase] = useNodesState<StoredWorkflowNode>(initialGraph.nodes);
  const [edges, setEdges, onEdgesChangeBase] = useEdgesState<Edge>(initialGraph.edges);
  const [selectedNode, setSelectedNode] = useState<StoredWorkflowNode | null>(null);
  const [lastSavedAt, setLastSavedAt] = useState<Date | null>(
    initialGraph.updatedAt && new Date(initialGraph.updatedAt).getTime() > 0 ? new Date(initialGraph.updatedAt) : null
  );
  ...
  useEffect(() => {
    ...
    setLastSavedAt(graph.updatedAt && new Date(graph.updatedAt).getTime() > 0 ? new Date(graph.updatedAt) : null);
  ...
  const handlePaletteClick = useCallback(
    (item: PaletteItem) => {
      ...
      const screenPoint = bounds
        ? { x: bounds.left + bounds.width / 2 + (cascade - 2.5) * 60, y: bounds.top + 140 + cascade * 30 }
        : { x: 400, y: 150 };
  ...
  const updateSelectedNodeData = useCallback(
    (patch: Partial<WorkflowNodeData> | ((prev: WorkflowNodeData) => Partial<WorkflowNodeData>)) => {
      setSelectedNode((prevSelected) => {
        ...
        setNodes((nds) => nds.map((n) => (n.id === prevSelected.id ? { ...n, data: nextData } : n)));
  ...
  const updateSelectedNodeConfig = useCallback(
    (configPatch: Partial<WorkflowNodeData['config']>) => {
      updateSelectedNodeData((prevData) => ({ config: { ...prevData.config, ...configPatch } }));
    },
    ...
    <div className="h-[calc(100vh-3.5rem)] flex flex-col">
      <div className="h-14 border-b border-border bg-card flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild className="h-8 w-8 -ml-2"><Link href="/workflows"><ArrowLeft className="w-4 h-4" /></Link></Button>
          <div>
            ...
            <div className="text-[10px] text-muted-foreground mt-0.5 h-3 timecode">
              {lastSavedAt ? `Saved ${formatDistanceToNowStrict(lastSavedAt, { addSuffix: true })}` : 'Not saved yet'}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex">
        <div ref={canvasRef} className="flex-1 relative bg-background/50" onDragOver={handleDragOver} onDrop={handleDrop}>
          <ReactFlow
            nodes={nodes}
            edges={edges}
            ...
            {selectedNode ? (
              ...
              >
                <div className="p-4 border-b border-border">
                  ...
                  <Badge variant="outline" className="text-[9px] h-4 mt-2 capitalize">{selectedNode.data.kind}</Badge>
                </div>
      ...
      <Dialog open={testRunOpen} onOpenChange={setTestRunOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Simulated dry run of "{currentWorkflow?.name ?? 'this workflow'}" — no real actions were executed.
            </DialogDescription>
          </DialogHeader>
          ...
          {testRunResult && !testRunResult.hasTrigger && (
            <div className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              ...
              <span>No trigger node found — this workflow would never fire.</span>
            </div>
          )}

          {testRunResult && nodes.length === 0 && (
            <div className="text-xs text-muted-foreground">This workflow is empty — add nodes to test it.</div>
          )}

          {testRunResult && testRunResult.steps.length > 0 && (
            <div className="space-y-1.5">
              <div className="text-xs font-medium text-muted-foreground">Firing order</div>
              {testRunResult.steps.map((step, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-5 h-5 rounded flex items-center justify-center bg-${step.color}-500/10 text-${step.color}-500 shrink-0`}>
                      <Icon className="w-3 h-3" />
                    </div>
                    <div className="text-xs font-medium truncate">{step.label}</div>
                    <Badge variant="outline" className="text-[9px] h-4 ml-auto capitalize shrink-0">{step.kind}</Badge>
                  </motion.div>
                );
              })}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {testRunResult.unreachable.map((n) => (
                  <Badge key={n.id} variant="outline" className="text-[9px] h-5 border-amber-500/30 text-amber-500">
                    {n.data.label}
                  </Badge>
                ))}
              </div>
          ...
          <DialogFooter>
            <Button size="sm" onClick={() => setTestRunOpen(false)}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
