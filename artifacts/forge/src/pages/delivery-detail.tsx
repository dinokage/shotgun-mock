...
import { ShieldCheck, Lock, Key, Download, Play, Ban, Clock, ArrowLeft, PackageX, Package, Building2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useAuthStore } from '@/store/auth';
import { useDeliveryStore, isDeliveryActive, isDeliveryExpired, type Delivery } from '@/store/deliveries';
 ...
 */
function formatExpiryCountdown(expiresAt: string): { label: string; urgent: boolean } {
  const msRemaining = new Date(expiresAt).getTime() - Date.now();
  const daysRemaining = Math.ceil(msRemaining / (24 * 60 * 60 * 1000));
  if (daysRemaining <= 1) return { label: 'Access expires today', urgent: true };
  if (daysRemaining <= 3) return { label: `Access expires in ${daysRemaining} days`, urgent: true };
...
function ForgeMark({ className }: { className?: string }) {
  return (
    <div className={cn('bg-primary rounded-lg flex items-center justify-center shrink-0', className)}>
...
function DeliveryMasthead({ delivery, projectName }: { delivery: Delivery; projectName: string | undefined }) {
  return (
    <div className="flex items-center gap-2.5 text-center flex-col">
      ...
      <div>
        ...
        <div className="text-xs text-zinc-500">{projectName ?? delivery.projectId} • {delivery.clientName}</div>
      </div>
    </div>
  );
}
...
export default function DeliveryDetail() {
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-zinc-950 gap-4 p-4">
        ...
        <div className="flex flex-col items-center gap-3 text-center max-w-sm">
          ...
          <div>
            <h1 className="text-white font-semibold">This delivery link doesn't exist</h1>
            <p className="text-zinc-500 text-sm mt-1">
              It may have been mistyped, or the delivery it pointed to was removed. Contact the studio if you believe this is a mistake.
            </p>
          </div>
        </div>
  ...
  if (!unlocked) {
    const expiryHint = delivery.expiresAt && isDeliveryActive(delivery) ? formatExpiryCountdown(delivery.expiresAt) : null;
    return (
          ...
          <CardContent className="space-y-4">
            ...
            {expiryHint && (
              <p className={cn(
                ...
                expiryHint.urgent ? 'text-status-orange' : 'text-muted-foreground'
              )}>
                <Clock className="w-3.5 h-3.5" />
                {expiryHint.label} — <span className="timecode">{new Date(delivery.expiresAt!).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="w-full max-w-md border-border/50 shadow-2xl">
          <CardHeader className="space-y-2 text-center pb-6">
            <div className={cn(
              ...
              {expired ? <Clock className="w-6 h-6 text-status-orange" /> : <Ban className="w-6 h-6 text-status-red" />}
            </div>
            <CardTitle className="text-xl font-bold">
              {expired ? 'This delivery link has expired' : 'This delivery link has been disabled'}
            </CardTitle>
            <CardDescription className="max-w-sm mx-auto">
              {expired ? (
                <>
                  ...
                  {delivery.expiresAt && (
                    <>on <span className="timecode text-foreground/80">{new Date(delivery.expiresAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>{' '}</>
                  )}
                  to keep the files scoped to the review window. The files themselves haven't changed — you just need a fresh link.
                </>
              ) : (
          ...
          <CardContent>
            <p className="text-sm text-center text-muted-foreground">
              Contact <span className="text-foreground font-medium">{delivery.createdByName}</span> at the studio for a new link.
            </p>
          </CardContent>
        </Card>
        {currentUser && (
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <header className="h-16 px-6 flex items-center justify-between border-b border-white/10 bg-zinc-900/60 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-zinc-400 truncate flex items-center gap-1.5">
              ...
              {project?.name} • Delivered to {delivery.clientName} by {delivery.createdByName}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {delivery.expiresAt ? (
              ...
              return (
                <Badge
                  ...
                  className={cn(
                    ...
                    expiryInfo.urgent ? 'border-status-orange/30 text-status-orange bg-status-orange/10' : 'border-status-green/30 text-status-green bg-status-green/10'
                  )}
                >
            ...
            <Badge variant="outline" className="hidden sm:inline-flex gap-1.5 font-normal border-white/10 text-zinc-400">
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="flex items-center gap-1.5 text-xs text-zinc-500">
          <Lock className="w-3 h-3" /> Secure delivery link — access code required, not tied to a Forge account
        </div>
        ...
        {activeShot ? (
          ...
          <Empty>
            <EmptyHeader>
              ...
              <EmptyDescription>This package doesn't contain any shots yet. Check back after the studio adds content.</EmptyDescription>
            </EmptyHeader>
          </Empty>
        ) : (
