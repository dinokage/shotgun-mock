...
export default function Scheduling() {
  ...
  const handleStartImport = () => {
    ...
    const assignees = USERS.filter(
      (u) => u.departmentId === dept.id && u.role !== "client",
    );
    const assigner =
      USERS.find(
        (u) =>
    ...
    const imported: Task[] = Array.from({ length: importedCount }).map(
      (_, i) => ({
    ...
    toast({
      ...
      description: `Imported ${imported.length} shots into ${project.name}.`,
    });
  };
              ...
              <div className="space-y-6 py-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Map your spreadsheet columns to Forge fields to bulk import
                    Shots and Tasks.
                  </p>
                </div>
