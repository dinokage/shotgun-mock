    ...
    return tasks
      ...
      .map((t) => {
        ...
        return {
          ...
          visibleDuration: isBeforeStart
            ? duration - getDaysDiff(startDate, timelineStartStr)
            : duration,
        };
      });
      ...
      <div className="flex-1 flex overflow-hidden bg-background">
            ...
            {Object.keys(groupedByUser).map((userId) => {
              ...
              return (
                <div
                  ...
                  style={{ height: `${Math.max(48, rows.length * 40 + 8)}px` }}
                >
                  <div className="flex items-center gap-3 p-3 absolute left-0 w-64 bg-card/80 backdrop-blur-sm pointer-events-none">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="text-sm font-medium truncate pointer-events-auto leading-tight">
                        {user?.name}
                      </div>
        ...
        <div className="flex-1 overflow-auto relative custom-scrollbar">
          <div className="h-12 border-b border-border flex sticky top-0 bg-card/95 backdrop-blur z-20 w-max">
            {dates.map((date, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex flex-col items-center justify-center border-r border-border ${weekend ? "bg-muted/30" : ""} ${isToday ? "bg-primary/10 text-primary" : ""}`}
                  style={{
                    width: `${cellWidth}px`,
          ...
          >
            <div className="absolute inset-0 flex pointer-events-none z-10">
              {dates.map((date, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`relative h-full border-r border-border/50 ${date.getDay() === 0 || date.getDay() === 6 ? "bg-muted/10" : ""}`}
                    style={{
                      width: `${cellWidth}px`,
                      minWidth: `${cellWidth}px`,
                    }}
            ...
            <svg
              className="absolute top-0 left-0 pointer-events-none z-10 h-full"
              style={{ width: `${dependencyBoundsWidth}px`, minHeight: "100%" }}
            >
              <defs>
                <marker
                  id="arrowhead"
            ...
            {Object.keys(groupedByUser).map((userId) => {
              ...
              return (
                <div
                  key={userId}
                  className="relative border-b border-border hover:bg-muted/5 group transition-colors"
                  style={{ height: `${rowHeight}px` }}
                >
                  {userLeave.map((leave) => {
                    const leaveOffset = dayOffset(
                      timelineStartStr,
                      leave.start,
                    );
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          ...
                          bottom: "4px",
                          background:
                            "repeating-linear-gradient(45deg, rgba(239, 68, 68, 0.05), rgba(239, 68, 68, 0.05) 10px, rgba(239, 68, 68, 0.15) 10px, rgba(239, 68, 68, 0.15) 20px)",
                        ...
                        title={leave.reason}
                      >
                        <span className="text-[10px] font-bold text-red-500/70 uppercase tracking-widest bg-background/50 px-1 py-0.5 rounded backdrop-blur-sm">
                  ...
                  {rows.map((task, i) => {
                    ...
                    return (
                      <DraggableBar
                        ...
                        onReschedule={(dayDelta) =>
                          handleReschedule(task.id, dayDelta)
                        }
                      />
                    );
                  })}
...
function DraggableBar({
  ...
  return (
    <motion.div
      ...
      whileHover={
        !isDragging
          ? { y: -2, boxShadow: "0 6px 16px rgba(0,0,0,0.3)" }
          : undefined
      }
      whileTap={{ scale: 0.98 }}
      onClick={handleClick}
    ...
    >
      <div className="px-2 py-1 text-[10px] text-white font-medium truncate flex items-center justify-between h-full pointer-events-none">
        ...
        {width > 120 && (
          <span className="opacity-80 shrink-0 ml-2">{project?.name}</span>
        )}
      </div>
    </motion.div>
  );
