...
import {
  ...
  useSensor, useSensors, useDroppable, useDndContext, DragStartEvent, DragEndEvent,
} from '@dnd-kit/core';
...
function capacityColor(cap: number) {
  return cap > 90 ? 'bg-red-500' : cap > 75 ? 'bg-yellow-500' : 'bg-emerald-500';
}
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="mb-2 cursor-grab active:cursor-grabbing outline-none">
      ...
      >
        ...
        <div className="pl-2 text-xs font-semibold leading-tight text-foreground/90 truncate">{task.title}</div>
        <div className="pl-2 flex items-center justify-between">
          <PriorityChip priority={task.priority} className="text-[9px] px-1.5 py-0" />
...
function BoardColumn({
  id, name, subtitle, avatar, capacity, tasks, isPool,
}: {
  id: string; name: string; subtitle?: string; avatar?: string; capacity?: number;
  tasks: Task[]; isPool?: boolean;
}) {
  ...
  const isDropTarget = isOver || (over != null && (over.id === id || tasks.some((t) => t.id === over.id)));
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isPool ? 'bg-muted/20 border-dashed border-border' : 'bg-muted/30 border-border',
    ...
    >
      <div className="p-3 border-b border-border rounded-t-lg bg-muted/50 flex items-center gap-2">
        {isPool ? (
          ...
          <Avatar className="w-7 h-7 shrink-0"><AvatarImage src={avatar} /><AvatarFallback className="text-xs">{name.charAt(0)}</AvatarFallback></Avatar>
        )}
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold truncate leading-tight">{name}</div>
          <div className="text-[10px] text-muted-foreground truncate leading-tight">{subtitle}</div>
        </div>
        <span className="text-xs bg-background px-2 py-0.5 rounded-full text-muted-foreground shrink-0">{tasks.length}</span>
      </div>
...
export default function TeamBoard() {
  ...
  const people = useMemo(() => {
    return USERS
      ...
      .filter((u) => departmentFilter === 'all' || u.departmentId === departmentFilter)
      .map((user) => ({ user, tasks: filteredTasks.filter((t) => t.assigneeId === user.id) }))
      .filter((col) => showEmpty || col.tasks.length > 0)
      .sort((a, b) => b.tasks.length - a.tasks.length || b.user.capacity - a.user.capacity);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (targetUserId === '') {
      ...
      toast({ title: 'Task unassigned', description: `${activeTaskItem.title} moved back to the unassigned pool.` });
    } else {
      const person = USERS.find((u) => u.id === targetUserId);
      reassignTask(activeTaskItem.id, targetUserId);
      toast({ title: 'Task assigned', description: `${activeTaskItem.title} assigned to ${person?.name ?? 'team member'}.` });
    }
  };
    ...
    <div className="flex flex-col h-full">
      <div className="border-b border-border bg-card/50 px-6 py-3 shrink-0 flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Drag a task card onto a person to assign it &middot; drag onto Unassigned to release it</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Select value={departmentFilter} onValueChange={setDepartmentFilter}>
            <SelectTrigger className="w-[190px] h-9"><SelectValue placeholder="All Departments" /></SelectTrigger>
            <SelectContent>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="flex-1 flex overflow-x-auto p-4 gap-4 pb-8 bg-background">
          <BoardColumn id={UNASSIGNED} name="Unassigned" subtitle="Needs an owner" tasks={unassignedTasks} isPool />
