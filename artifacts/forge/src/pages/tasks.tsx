...
import { Search, ListTodo, LayoutGrid, List, AlertTriangle, Circle, Play, UserCheck, Clock, CheckCircle2 } from 'lucide-react';
...
export default function Tasks() {
  ...
  const [view, setView] = useState<ViewMode>(() => {
    ...
    return role && ['artist', 'senior_artist', 'junior_artist'].includes(role) ? 'kanban' : 'list';
  });
  ...
  const isArtist = currentUser ? ['artist', 'senior_artist', 'junior_artist'].includes(currentUser.role) : true;
  ...
  }, [search, statusFilter, projectFilter, departmentFilter, myTasksOnly, forceMyTasksOnly, currentUserId, liveTasks, needsReviewOnly, myDepartmentName]);
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isLeadOrSupervisor && (
            ...
            >
              ...
              Needs My Review
              {needsReviewCount > 0 && (
                <Badge variant="secondary" className={cn('px-1.5 py-0 text-[10px] h-4', needsReviewOnly ? 'bg-black/10 text-accent-tally-foreground' : 'bg-accent-tally/15 text-accent-tally')}>
                  {needsReviewCount}
                </Badge>
              )}
            </Button>
          ...
          {!isArtist && (
            <>
              <Button variant={myTasksOnly ? 'default' : 'outline'} size="sm" onClick={() => setMyTasksOnly(!myTasksOnly)} className="gap-1.5">
              ...
              <div className="flex bg-muted/50 p-0.5 rounded-lg border border-border">
                <Button variant={view === 'list' ? 'secondary' : 'ghost'} size="sm" onClick={() => setView('list')} className="h-7 px-2.5 shadow-none"><List className="w-4 h-4" /></Button>
                <Button variant={view === 'kanban' ? 'secondary' : 'ghost'} size="sm" onClick={() => setView('kanban')} className="h-7 px-2.5 shadow-none"><LayoutGrid className="w-4 h-4" /></Button>
              </div>
        ...
        {['all', 'todo', 'in_progress', 'review', 'done'].map(s => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
              statusFilter === s ? 'bg-primary text-primary-foreground shadow-sm hover:bg-primary/90' : 'bg-muted/50 text-muted-foreground hover:bg-muted'
            }`}
          >
      ...
      <div className="flex flex-wrap items-center gap-3 shrink-0">
        ...
        <Select value={projectFilter} onValueChange={setProjectFilter}>
          ...
          <SelectContent>
            ...
            {liveProjects.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={departmentFilter} onValueChange={setDepartmentFilter}>
          ...
          <SelectContent>
            ...
            {liveDepartments.map(d => <SelectItem key={d.name} value={d.name}>{d.name}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
        ...
        {view === 'kanban' ? (
          ...
          <div className="rounded-md border border-border overflow-y-auto h-full p-3 space-y-3">
            {filtered.slice(0, 100).map(task => {
              ...
              return (
                <Card key={task.id} onClick={() => setActiveTaskDrawer(task.id)} className="p-4 cursor-pointer touch-target hover-elevate active-elevate-2 border-border">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-3 pt-3 border-t border-border/60">
                    <span className="text-xs text-muted-foreground timecode">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : 'No date'}</span>
                    {task.status === 'in_progress' && task.assigneeId === currentUser?.id && (
                      <Button size="sm" variant="outline" className="h-8 text-[11px] bg-blue-500/10 text-blue-500 border-blue-500/20 hover:bg-blue-500/20 touch-target" onClick={(e) => handleSubmitForReview(e, task.id)}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.slice(0, 100).map(task => {
                  ...
                  const project = liveProjects.find(p => p.id === task.projectId);
                  return (
                    <tr key={task.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => setActiveTaskDrawer(task.id)}>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        {assignee && (
                          <div className="flex items-center gap-2">
                            <Avatar className="w-6 h-6"><AvatarImage src={assignee.avatar} /><AvatarFallback className="text-[10px]">{assignee.name.charAt(0)}</AvatarFallback></Avatar>
                      ...
                      <td className="p-4 text-muted-foreground font-mono text-xs">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : ''}</td>
                      <td className="p-4 text-right">
                        {task.status === 'in_progress' && task.assigneeId === currentUser?.id && (
                          <Button size="sm" variant="outline" className="h-7 text-[10px] bg-blue-500/10 text-blue-500 border-blue-500/20 hover:bg-blue-500/20 mr-2" onClick={(e) => handleSubmitForReview(e, task.id)}>
                            <Play className="w-3 h-3 mr-1" /> Submit
                          </Button>
                        )}
                      </td>
