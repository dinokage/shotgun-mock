...
import { TASKS, isTaskActive, isTaskDone } from "@/data/mockData";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {DEPARTMENTS.length} departments · VFX production pipeline
          </p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {(["PROD", "3D", "VFX", "2D"] as const).map((pipelineType) => {
          ...
          const depts = DEPARTMENTS.filter(
            (d) =>
              d.pipeline === pipelineType &&
              (isGlobalManager || d.id === currentUser?.departmentId),
          );
