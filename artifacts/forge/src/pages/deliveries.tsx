...
function statusBadge(delivery: { status: string } & Parameters<typeof isDeliveryActive>[0]) {
  ...
  return <Badge variant="outline" className="border-red-500/30 text-red-500">Revoked</Badge>;
}
...
export default function Deliveries() {
  ...
  const eligibleShots = useMemo(
    () => shots.filter((s) => s.projectId === projectId && (DELIVERY_ELIGIBLE_STATUSES as readonly string[]).includes(s.status)),
    [shots, projectId]
  );
  ...
  const handleCreate = () => {
    if (!currentUser || !selectedProject || selectedShotIds.size === 0 || !title.trim()) return;

    const items: DeliveryItem[] = eligibleShots
      .filter((s) => selectedShotIds.has(s.id))
      .map((s) => ({ shotId: s.id, name: s.name, thumbnailSeed: s.thumbnailSeed }));
    ...
    toast({ title: 'Delivery Created', description: `"${delivery.title}" is ready to share — access code ${delivery.accessCode}.` });
    setCreateOpen(false);
    resetForm();
  };
  ...
  const handleCopyLink = async (id: string, accessCode: string) => {
    ...
    toast(
      ok
        ? { title: 'Link Copied', description: 'Delivery link and access code copied to clipboard.' }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Final, approved packages sent to clients — scoped and expiring, unlike the ongoing Client Review portal.
          </p>
        </div>
        <Dialog open={createOpen} onOpenChange={(open) => { setCreateOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild>
            <Button className="gap-2"><Plus className="w-4 h-4" /> New Delivery</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>New Delivery</DialogTitle>
              <DialogDescription>Package final approved shots into a shareable, expiring client link.</DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2 max-h-[60vh] overflow-y-auto pr-1">
              ...
              {projectId && (
                <>
                  <div className="space-y-2">
                    <Label>Title</Label>
                    <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Trailer Cut — Final Delivery" />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label>Shots to include ({selectedShotIds.size} selected)</Label>
                    {eligibleShots.length === 0 ? (
                      <p className="text-xs text-muted-foreground">No finished (complete, approved, or published) shots in this project yet.</p>
                    ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={String(expiryDays)} onValueChange={(v) => setExpiryDays(v === 'null' ? null : Number(v))}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="e.g. Masters only, ProRes 4444." />
                  </div>
                </>
              )}
            </div>
            <DialogFooter>
              ...
              <Button onClick={handleCreate} disabled={!projectId || selectedShotIds.size === 0 || !title.trim()}>
                Create Delivery
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      {deliveries.length === 0 ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>Create one to package approved shots into a shareable client link.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
