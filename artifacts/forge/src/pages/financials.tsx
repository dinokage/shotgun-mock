...
function getProjectFinancials(p: Project): ProjectFinancials {
  ...
  const monthsElapsed = Math.max(
    1,
    (MOCK_TODAY.getFullYear() - start.getFullYear()) * 12 +
      (MOCK_TODAY.getMonth() - start.getMonth()),
  );
 ...
 */
function getStudioSpendTrend(
  projects: Project[],
  financials: Map<string, ProjectFinancials>,
  steps = SPEND_TREND_STEPS,
): number[] {
  if (projects.length === 0) return [];
  const todayTime = MOCK_TODAY.getTime();
  const earliestStart = Math.min(
    ...projects.map((p) => new Date(p.startDate).getTime()),
  );
...
export default function FinancialDashboard() {
  ...
  const totalMonthlyBurn = useMemo(
    () =>
      activeProjects.reduce(
        (acc, p) => acc + (projectFinancials.get(p.id)?.monthlyBurn || 0),
        0,
      ),
  ...
  const departmentSpend = useMemo(() => {
    return departments.slice(0, 6).map((d) => {
      ...
      const actualHours = deptTasks.reduce(
    ...
    <div className="p-8 h-[calc(100vh-3.5rem)] overflow-auto bg-background text-foreground">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Studio budget and cost tracking, derived from each project's real
          progress and risk profile.
        </p>
      </div>
          ...
          <Card className="bg-card border-border transition-colors hover:border-primary/40 h-full">
            ...
            <CardContent>
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                {overallPercentSpent.toFixed(0)}% of total budget &middot; spend
                trend across the studio
              </p>
            </CardContent>
          </Card>
        </motion.div>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground tabular-nums">
                <CountUp
                  ...
                  format={(n) => `$${Math.round(n / 1000)}k/mo`}
                />
              </div>
              ...
              <p className="text-xs text-muted-foreground mt-1 flex items-center">
                <TrendingUp className="w-3 h-3 text-orange-500 mr-1" /> Blended
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between text-sm mb-1">
                      ...
                      <span
                        className={`font-mono text-xs ${pf.isOverBudget ? "text-red-400" : "text-muted-foreground"}`}
                      >
                        ${(pf.spent / 1000000).toFixed(1)}M / $
                        {((p.budget || 0) / 1000000).toFixed(1)}M
                      </span>
                    </div>
                    ...
                    {pf.isOverBudget && (
                      <div className="text-[10px] text-red-400 mt-1">
                        Over budget by $
                        {Math.abs(pf.remaining / 1000).toFixed(0)}k
                      </div>
                    )}
                  </motion.div>
                );
              ...
              {departmentSpend.map((d, i) => (
                ...
                >
                  ...
                  <div
                    ...
                    title={`${d.actualHours}h logged / ${d.estimatedHours}h bid`}
                  >
                    <motion.div
                  ...
                  <tbody className="divide-y divide-border">
                    {tasks.slice(0, 10).map((task, i) => {
                      ...
                      if (task.status === "complete") {
                        actualDays =
                          bidDays + ((hashString(task.title) % 5) - 2);
                      } else if (
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 text-right text-foreground font-medium">
                            {actualDays > 0 ? `${actualDays}d` : "-"}
                          </td>
                          <td
                            className={`p-4 text-right font-medium ${isOverBudget ? "text-red-500" : variance > 0 ? "text-emerald-500" : "text-muted-foreground"}`}
                          >
                            {actualDays === 0
                              ? "-"
                              : variance > 0
