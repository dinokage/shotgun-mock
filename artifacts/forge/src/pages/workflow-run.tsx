...
function buildRunSteps(run: WorkflowRunState): RunStepView[] {
  ...
  for (const entry of run.logs) {
    ...
    else if (entry.status === 'success') statusByNode.set(entry.node, 'complete');
    else if (!statusByNode.has(entry.node)) statusByNode.set(entry.node, 'running');
  }

  if (run.status === 'running' && order.includes(run.currentNode) && statusByNode.get(run.currentNode) !== 'rejected') {
...
function headerBadge(status: WorkflowRunState['status']) {
  switch (status) {
    case 'completed':
      return <Badge variant="outline" className="text-status-green border-status-green/30 bg-status-green/10">Completed</Badge>;
    case 'failed':
      return <Badge variant="outline" className="text-destructive border-destructive/30 bg-destructive/10">Rejected</Badge>;
    default:
      return <Badge variant="outline" className="text-blue-500 border-blue-500/30 bg-blue-500/10">In Progress</Badge>;
  }
}
...
export default function WorkflowRun() {
  ...
  const workflow = workflowId ? WORKFLOWS.find((wf) => wf.id === workflowId) : undefined;

  const { toast } = useToast();
  const currentUser = useAuthStore((s) => s.currentUser);
  const canApprove = useCapability('approve_reviews');

  const run = useWorkflowsStore((s) => (workflowId ? s.getRun(workflowId) : undefined));
    ...
    return (
      <div className="flex flex-col h-screen bg-background">
        ...
        <Empty className="flex-1">
          <EmptyHeader>
            <EmptyMedia variant="icon"><AlertCircle className="w-6 h-6" /></EmptyMedia>
            <EmptyTitle>Workflow not found</EmptyTitle>
            <EmptyDescription>This workflow doesn't exist or may have been removed.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      </div>
    );
  ...
  const handleReject = () => {
    ...
    toast({ title: 'Step rejected', description: `${gateStep.label} was rejected. The run has been halted.`, variant: 'destructive' });
  };
      ...
      <div className="flex-1 bg-muted/10 relative overflow-hidden flex flex-col">
        <div className="flex-1 relative overflow-auto">
          {steps.length === 0 ? (
            <Empty className="h-full">
              <EmptyHeader>
                <EmptyMedia variant="icon"><PlayCircle className="w-6 h-6" /></EmptyMedia>
                <EmptyTitle>No runs yet</EmptyTitle>
                <EmptyDescription>This workflow hasn't been triggered yet, so there's nothing to show.</EmptyDescription>
              </EmptyHeader>
            </Empty>
          ) : (
            <>
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ minWidth: STEP_START + steps.length * STEP_GAP + 160 }}>
              ...
              {steps.map((step) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white mb-3 ${color} ${step.status === 'running' ? 'animate-spin' : ''}`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div className="font-medium text-sm">{step.label}</div>
                    <div className="text-xs text-muted-foreground capitalize mt-1">{step.status}</div>
        ...
        <div className="h-64 border-t border-border bg-card flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 font-mono text-xs space-y-1.5">
            {run.logs.length === 0 ? (
              <div className="text-muted-foreground">No log entries for this run yet.</div>
            ) : (
