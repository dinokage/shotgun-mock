...
export default function Assets() {
  ...
  const filtered = useMemo(() => {
    return assets.filter((a) => {
      ...
      if (projectFilter !== "all" && a.projectId !== projectFilter)
        return false;
      return true;
    });
  }, [
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={setNewAssetProjectId}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <Card className="overflow-hidden hover:shadow-lg transition-all cursor-pointer group border-border hover:border-primary/40">
                      <div className="aspect-video bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center relative">
                        ...
                        <div className="absolute top-2 right-2">
                          <Badge
                            className={`${STATUS_COLORS[asset.status]} text-[9px] border-0`}
                          >
                        ...
                        <div className="absolute bottom-2 left-2">
                          <Badge
                            className={`${TYPE_COLORS[asset.type] || ""} text-[9px] border`}
                          >
                            {asset.type}
                          </Badge>
                        </div>
                ...
                {filtered.slice(0, listVisible).map((asset) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <Badge
                          className={`${TYPE_COLORS[asset.type] || ""} text-[10px] border`}
                        >
                          {asset.type}
                        </Badge>
                      </td>
                      <td className="p-4">
                        <Badge
                          className={`${STATUS_COLORS[asset.status]} text-[10px]`}
                        >
                          {asset.status.replace("-", " ")}
                        </Badge>
                      </td>
      ...
      {filtered.length === 0 && (
        <Empty className="border-2 border-dashed border-border rounded-lg py-20">
          <EmptyHeader>
            ...
            <EmptyDescription>
              No assets match your current search and filters.
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button
