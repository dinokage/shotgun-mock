                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        setFilter({
                          ...filter,
                          artist: toggle(filter.artist, a.id),
                        })
                      }
                    />
...
export default function EpisodesTab({ project }: { project: any }) {
  ...
  const activeSequence =
    allSequences.find((s) => s.id === activeSequenceId) ?? null;
  ...
  const matchesFilter = (shot: Shot, f: FilterState) => {
    if (f.dept.length) {
      const deptId = shot.assigneeId
        ? userById.get(shot.assigneeId)?.departmentId
        : null;
  ...
  const availableFor = (scopedShots: Shot[]) => {
    const artistIds = new Set(
      scopedShots.map((s) => s.assigneeId).filter(Boolean),
    );
  ...
  const handleAssign = async (
    ...
    try {
      ...
      toast({
        title: "Assigned",
        description: `${shot.name} assigned to ${artistName}.`,
      });
    } catch (err: any) {
      toast({
    ...
    >
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            variant="outline"
            onClick={() => setAddDialogLevel(null)}
            disabled={isCreating}
          >
            Cancel
          </Button>
  ...
  if (effectiveLevel === "episodes") {
    const episodeShotsOf = (episodeId: string) =>
      shots.filter((s) => s.episodeId === episodeId);
    const episodeSeqCount = (episodeId: string) =>
      allSequences.filter((sq) => sq.episodeId === episodeId).length;

    const filteredEpisodes = episodes.filter((ep) => {
      if (
        episodeFilter.search &&
        !ep.name.toLowerCase().includes(episodeFilter.search.toLowerCase())
      )
        return false;
      if (episodeFilter.dept.length || episodeFilter.artist.length) {
        return episodeShotsOf(ep.id).some((s) =>
          matchesFilter(s, episodeFilter),
        );
      }
      return true;
    });
  ...
  if (effectiveLevel === "sequences" && activeEpisode) {
    const sequencesInEpisode = allSequences.filter(
      (sq) => sq.episodeId === activeEpisode.id,
    );
    const sequenceShotsOf = (sequenceId: string) =>
    ...
    const filteredSequences = sequencesInEpisode.filter((sq) => {
      ...
      if (sequenceFilter.dept.length || sequenceFilter.artist.length) {
        return sequenceShotsOf(sq.id).some((s) =>
          matchesFilter(s, sequenceFilter),
        );
      }
      return true;
    });

    const { depts, artistsList } = availableFor(
      shots.filter((s) => s.episodeId === activeEpisode.id),
    );
  ...
  if (effectiveLevel === "shots" && activeEpisode && activeSequence) {
    const shotsInSequence = shots.filter(
      (s) => s.sequenceId === activeSequence.id,
    );
    ...
    const selectedShot =
      filteredShots.find((s) => s.id === selectedShotId) ?? null;
                      ...
                      <ContextMenu key={shot.id}>
                        ...
                        <ContextMenuContent className="w-56">
                          ...
                          {departments.map((dept) => {
                            const deptArtists = artists.filter(
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      {users.find((u) => u.id === selectedShot.assigneeId)
                        ?.name ?? "Unassigned"}
                    </div>
                  </div>
                  <div>
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        ...
        <div
          className="font-medium text-sm truncate flex items-center justify-between"
          title={name}
        >
