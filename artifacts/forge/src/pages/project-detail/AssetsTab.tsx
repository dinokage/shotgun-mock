...
export default function AssetsTab({ project }: { project: any }) {
  ...
  const items = allItems.filter(i =>
    ...
    (query === '' || i.name.toLowerCase().includes(query) || i.id.toLowerCase().includes(query))
  );
          ...
          {items.length === 0 ? (
            <Empty className="h-full">
              ...
              {(activeFilterCount > 0 || query !== '') && (
                <EmptyContent>
                  <Button variant="outline" size="sm" onClick={() => { setStatusFilter([]); setTypeFilter([]); setSearch(''); }}>
                    Clear filters
                  </Button>
                </EmptyContent>
              )}
            ...
            <div className="p-4 space-y-6">
              ...
              <div className="pt-4 border-t border-border">
                ...
                {recentVersions.length === 0 ? (
                  <p className="text-xs text-muted-foreground">No versions submitted yet.</p>
                ) : (
                  <div className="space-y-2">
                    {recentVersions.map(v => {
                      const author = USERS.find(u => u.id === v.createdById);
                      return (
                        <div key={v.id} className="text-xs p-2 border border-border rounded bg-muted/30">
