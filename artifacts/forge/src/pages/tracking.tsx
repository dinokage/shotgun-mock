  ...
  switch (key) {
    ...
    case "status":
      return {
        ...
        color: STATUS_DOT_COLORS[row.status],
      };
    case "internalReview":
      return {
        id: `ir-${row.internalReview}`,
        label: humanize(row.internalReview),
        color: REVIEW_DOT_COLORS[row.internalReview],
      };
    case "clientReview":
      return {
        id: `cr-${row.clientReview}`,
        label: humanize(row.clientReview),
        color: REVIEW_DOT_COLORS[row.clientReview],
      };
    default:
...
function buildGroups(
  ...
  const map = new Map<
    string,
    { label: string; color?: string; rows: TrackingRow[] }
  >();
  rows.forEach((row) => {
...
function MiniStatusBar({ rows }: { rows: TrackingRow[] }) {
  const counts = useMemo(() => {
    ...
    rows.forEach((r) => {
      c[bucketShotStatus(r.status)]++;
    });
    return c;
  }, [rows]);
  const total = rows.length || 1;
  const order: (keyof typeof SHOT_BUCKET_COLOR)[] = [
    "complete",
    "in-progress",
    "review",
    "bottleneck",
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      <span className="text-[10px] text-emerald-500 font-mono w-8 text-right">
...
export default function TrackingGrid() {
  ...
  const markDirty = () => {
    if (!applyingRef.current && activeViewId) setActiveViewId(null);
  };
  ...
  const setGroupBy2 = (v: TrackingGroupKey) => {
    setGroupBy2State(v);
    setCollapsedKeys(new Set());
    markDirty();
  };
  const setSortBy = (v: TrackingSortKey) => {
    setSortByState(v);
    markDirty();
  };
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      const confirmed = window.confirm(
        "You have unsaved tracking grid changes that haven't been saved. Leave this page and discard them?",
      );
      if (!confirmed) {
        e.preventDefault();
        e.stopPropagation();
  ...
  const trackingData = useMemo(() => {
    ...
    filteredShots = [...filteredShots].sort((a, b) => {
      if (a.projectId !== b.projectId)
        return String(a.projectId || "").localeCompare(
          String(b.projectId || ""),
        );
      if (a.episodeId !== b.episodeId)
        return String(a.episodeId || "").localeCompare(
          String(b.episodeId || ""),
        );
      if (a.sequenceId !== b.sequenceId)
        return String(a.sequenceId || "").localeCompare(
          String(b.sequenceId || ""),
        );
    ...
    let mapped: TrackingRow[] = filteredShots.map((shot, i) => {
      ...
      return {
        ...
        internalReview:
          localOverrides[shot.id]?.internalReview || shot.internalReviewStatus,
        clientReview:
          localOverrides[shot.id]?.clientReview || shot.clientReviewStatus,
        usdVersion: shot.usdVersion || "v001.usd",
        updatedAt: shot.updatedAt,
        notes:
          localOverrides[shot.id]?.notes !== undefined
            ? localOverrides[shot.id]?.notes
            : shot.notes || "No notes.",
      };
    });
  ...
  if (!currentUser || !isLeadership) {
    return (
      <div className="p-8 flex flex-col items-center justify-center h-full text-muted-foreground">
        ...
        <p>
          You need Production or Supervisor privileges to access the Tracking
          Grid.
        </p>
      </div>
    );
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        <div className="bg-card p-2 rounded-sm border border-border">
          ...
          <select
            ...
            value={row.internalReview}
            onChange={(e) =>
              updateCell(row.id, "internalReview", e.target.value)
            }
          >
            {REVIEW_STATUS_OPTIONS.map((o) => (
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] p-4 bg-background text-foreground">
      ...
      <div className="flex items-center justify-between shrink-0 mb-4">
        ...
        <div className="flex items-center gap-2">
          {isDirty && (
            <div
              className="flex items-center gap-1.5 text-[11px] text-amber-400 mr-1"
              title="You have staged edits that haven't been saved yet"
            >
            ...
            {deptRollup.map(({ dept, total, buckets }, i) => (
              ...
              >
                  ...
                  <TooltipContent side="bottom" className="text-[11px]">
                    ...
                    {(
                      ...
                      <div key={key} className="flex justify-between gap-3">
                        ...
                        <span>
                          {buckets[key]} (
                          {Math.round((buckets[key] / total) * 100)}%)
                        </span>
                      </div>
                    ))}
                  </TooltipContent>
                ...
                <div className="flex justify-between mt-1.5 text-[9px] text-muted-foreground/70">
                  ...
                  {buckets.bottleneck > 0 && (
                    <span className="text-red-500 font-medium">
                      {buckets.bottleneck} blocked
                    </span>
                  )}
                </div>
              </motion.div>
      ...
      <div className="bg-card border-t border-border p-2 flex justify-between items-center text-xs text-muted-foreground">
        ...
        <div className="flex gap-4">
          <span className="flex items-center gap-1">
            ...
            Approved:{" "}
            {trackingData.filter((d) => d.clientReview === "approved").length}
          </span>
          <span className="flex items-center gap-1">
            <div className="w-2 h-2 rounded-full bg-orange-500"></div> Pending:{" "}
            {trackingData.filter((d) => d.clientReview === "pending").length}
          </span>
        </div>
      </div>
    </div>
