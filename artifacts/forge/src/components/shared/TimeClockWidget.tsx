...
export function TimeClockWidget() {
  ...
  const myTasks = currentUser ? tasks.filter((t) => t.assigneeId === currentUser.id && isTaskActive(t.status)) : [];
  ...
  useEffect(() => {
    ...
    setStartTime(start);
    setPunchedIn(true);
    setSelectedTaskId((current) => current || myTasks.find((t) => t.status === 'in-progress')?.id || myTasks[0]?.id || '');
  ...
  const handlePunchIn = () => {
    ...
    setSelectedTaskId(myTasks.find((t) => t.status === 'in-progress')?.id ?? myTasks[0]?.id ?? '');
    setNote('');
  };
  ...
  const handleConfirmPunchOut = () => {
    ...
    if (task && hours > 0) {
      ...
      toast({ title: 'Punched Out', description: `Logged ${hours}h to ${task.title}. You've been signed out.` });
    } else if (task) {
      toast({ title: 'Punched Out', description: "Session was too short to log any time. You've been signed out." });
    } else {
      toast({ title: 'Punched Out', description: "No task selected — time was not logged. You've been signed out." });
    }
    resetSession();
  ...
  if (!punchedIn) {
    return (
      ...
      >
        ...
        <span className="text-[11px] font-medium text-muted-foreground">Clock In</span>
      </button>
    );
  }
    ...
    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
      ...
      <PopoverContent align="end" className="w-72 space-y-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground timecode">{formatTime(seconds)} elapsed</div>
        </div>
        {myTasks.length > 0 ? (
          <>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Log this time to</label>
            ...
            />
            <Button size="sm" className="w-full gap-1.5" onClick={handleConfirmPunchOut} disabled={!selectedTaskId}>
              <LogOut className="w-3.5 h-3.5" /> Punch Out & Log Time
            </Button>
          </>
        ) : (
          <>
            <p className="text-xs text-muted-foreground">You have no assigned tasks to log this time against.</p>
            <Button size="sm" variant="outline" className="w-full gap-1.5" onClick={handleConfirmPunchOut}>
