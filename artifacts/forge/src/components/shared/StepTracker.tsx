 ...
 * (not-started/todo/in-progress -> review/lead-review -> producer-review ->
 * approved), so this stays correct automatically if that pipeline changes
 * shape rather than needing a second source of truth.
 */
const STAGES = [
  {
    key: "submitted",
...
export function StepTracker({
  ...
  const statusSuffix = isCancelled
    ? ", cancelled"
    : isFlagged
      ? ", needs attention"
      : "";
              ...
              <div
                className={cn(
                  ...
                  "rounded-full flex items-center justify-center border transition-colors",
                    ...
                    "bg-accent-scope/15 border-accent-scope text-accent-scope",
                  active &&
                    !showFlag &&
                    !showCancelled &&
                    "bg-accent-tally/15 border-accent-tally text-accent-tally",
                  showFlag &&
