...
publishingRouter.get("/", async (req, res) => {
  try {
    ...
    if (typeof publishKind === "string" && publishKind)
      where.publishKind = publishKind;
    ...
    try {
      ...
      if (!PUBLISH_KINDS.includes(publishKind))
        return res.status(400).json({
          error: `publishKind must be one of: ${PUBLISH_KINDS.join(", ")}`,
        });
      ...
      if (!(await entityInTenant(entityType, entityId, tenantId)))
        return res
          .status(400)
          .json({ error: "entityId does not belong to this tenant" });
      if (taskId !== undefined && taskId !== null) {
        if (
          typeof taskId !== "string" ||
          !(await taskInTenant(taskId, tenantId))
        )
          return res
            .status(400)
            .json({ error: "taskId does not belong to this tenant" });
      }
      if (versionId !== undefined && versionId !== null) {
        if (
          typeof versionId !== "string" ||
          !(await versionInTenant(versionId, tenantId))
        )
          return res
            .status(400)
            .json({ error: "versionId does not belong to this tenant" });
      }
      ...
      if (validationLog !== undefined) {
        ...
        if (!parsed)
          return res.status(400).json({
            error:
              "validationLog must be an array of { name, passed, detail? }",
          });
        checks = parsed;
      }
