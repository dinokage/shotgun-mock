...
async function touchEntityType(id: string) {
  await prisma.entityTypeDef.update({
    where: { id },
...
schemaBuilderRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const desired =
        (optionalString(name) ?? "").trim() || "Untitled Entity Type";
...
schemaBuilderRouter.patch(
  ...
  async (req, res) => {
    try {
        ...
        if (trimmed !== current.name) {
          ...
          if (clash)
            return res.status(409).json({
              error: `An entity type named "${trimmed}" already exists`,
            });
        }
        data.name = trimmed;
      }
      ...
      if (color !== undefined)
        data.color = optionalString(color) ?? current.color;
      if (description !== undefined)
        data.description = optionalString(description) ?? "";
...
function validateFieldType(
  value: unknown,
): value is (typeof FIELD_TYPES)[number] {
  return (
    typeof value === "string" &&
  ...
  async (req, res) => {
    try {
      ...
      if (type !== undefined && !validateFieldType(type))
        return res
          .status(400)
          .json({ error: `type must be one of: ${FIELD_TYPES.join(", ")}` });
      ...
      if (key !== undefined) {
        ...
        if (RESERVED_KEYS.includes(resolvedKey))
          return res.status(400).json({
            error: `"${resolvedKey}" is a reserved word in computed expressions`,
          });
        if (taken.includes(resolvedKey))
          return res.status(409).json({
            error: `key "${resolvedKey}" is already used by another field`,
          });
      } else {
      ...
      const created = await prisma.entityFieldDef.create({
        data: {
          ...
          sortOrder: entityType.fields.reduce(
            (max, f) => Math.max(max, f.sortOrder + 1),
            0,
          ),
        },
      });
    ...
    try {
      ...
      if (key !== undefined) {
        ...
        if (RESERVED_KEYS.includes(normalized))
          return res.status(400).json({
            error: `"${normalized}" is a reserved word in computed expressions`,
          });
        if (normalized !== field.key) {
          ...
          if (clash)
            return res.status(409).json({
              error: `key "${normalized}" is already used by another field`,
            });
        }
        data.key = normalized;
      }
      ...
      if (defaultValue !== undefined)
        data.defaultValue = optionalString(defaultValue) ?? null;
      if (expression !== undefined)
        data.expression = optionalString(expression) ?? null;

      const updated = await prisma.entityFieldDef.update({
        where: { id: fieldId },
        data,
      });
      await touchEntityType(entityTypeId);
...
schemaBuilderRouter.put(
  "/entity-types/:id/fields/order",
  requireSchemaAdmin,
  async (req, res) => {
    try {
      const tenantId = req.tenantId!;
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "fieldIds must be an array of field ids" });

      const owned = new Set(entityType.fields.map((f) => f.id));
      if (
        fieldIds.length !== owned.size ||
        fieldIds.some((id: string) => !owned.has(id))
      )
        return res.status(400).json({
          error:
            "fieldIds must list every field of this entity type exactly once",
        });

      await prisma.$transaction(
        fieldIds.map((id: string, index: number) =>
          prisma.entityFieldDef.update({
            where: { id },
...
schemaBuilderRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        departmentId != null &&
        !(await departmentInTenant(String(departmentId), tenantId))
      )
      ...
      const created = await prisma.taskTemplateItem.create({
        data: {
          ...
          estimatedHours: Number.isInteger(estimatedHours)
            ? Math.max(0, estimatedHours)
            : 0,
          priority: optionalString(priority) ?? "medium",
          sortOrder: template.items.reduce(
...
schemaBuilderRouter.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!item)
        return res.status(404).json({ error: "Task template item not found" });
      ...
      if (estimatedHours !== undefined) {
        if (!Number.isInteger(estimatedHours) || estimatedHours < 0)
          return res
            .status(400)
            .json({ error: "estimatedHours must be a non-negative integer" });
      ...
      const updated = await prisma.taskTemplateItem.update({
        where: { id: itemId },
        data,
      });
      await touchTemplate(templateId);
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "itemIds must be an array of item ids" });

      const owned = new Set(template.items.map((i) => i.id));
      if (
        itemIds.length !== owned.size ||
        itemIds.some((id: string) => !owned.has(id))
      )
        return res.status(400).json({
          error: "itemIds must list every item of this template exactly once",
        });
